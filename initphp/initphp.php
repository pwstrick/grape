...
// 		$paths = array(
// 			$config['controller']['lib']
// 		);
// 		foreach ($paths as $path) {
// 			$phps = scandir($path);
// 			foreach ($phps as $file) {
// 				if ($file != '.' && $file != '..' && substr($file, -4, 4) == '.php') {
// 					//print_r(rtrim($path, '/') . '/' . $file);
// 					require_once(rtrim($path, '/') . '/' . $file);
// 				}
// 			}
// 		}
		//是否打开性能测试
	 ...
	 * @param boolean $need_app  需要引用getAppPath文件夹
	 * @return file
	 */
	public static function import($filename_old, array $pathArr = array(), $need_app = TRUE) {
		if($need_app) {
	...
	/**
	 * 载入文件，不需要引用getAppPath方法
	 */
	public static function importNoApp($filename_old, array $pathArr = array()) {
		return self::import($filename_old, $pathArr, false);
	}
	...
	public static function getService($servicename, $path = '') {
		...
		if (!InitPHP::importNoApp($file)) return false;
		return InitPHP::loadclass($class);
	}
	
	/**
	 * 获取Helper-实例并且单例模式获取Helper
	 * 1.单例模式获取
	 * 2.可以选定对应Helper路径path
	 * 3. helper需要在配置文件中配置参数，$path对应helper目录中的子目录
	 * 4. helper中聚集着函数，而不是一个一个的类
	 * 全局使用方法：InitPHP::getHelper($helper_name, $path = '')
	 * @param string $helper_name 帮助类名称
	 * @param string $path 模块名称
	 * @return boolean
	 */
	public static function getHelper($helper_name, $path = '') {
		$InitPHP_conf = InitPHP::getConfig();
		$path  = ($path == '') ? '' : $path . '/';
		$class = $helper_name . $InitPHP_conf['helper']['helper_postfix'];
		$file  = rtrim($InitPHP_conf['helper']['path'], '/') . '/' . $path . $class . '.php';
		if (!InitPHP::importNoApp($file)) return false;
		return true;
		//return InitPHP::loadclass($class);
	}
	...
	public static function getDao($daoname, $path = '') {
		...
		$path  = (($path == '') ? '' : $path) . '/';
		$file_name = $daoname . implode('', $other_pres) . $InitPHP_conf['dao']['dao_postfix'];//文件名字 用于初始化
		$class = $daoname. $InitPHP_conf['dao']['dao_postfix'];//类名
		...
		}elseif(strpos($path, 'search') === 0) {//全文检索
...
class Dao extends coreInit {
	 ...
	 * 初始化No-SQL
	 * @param string $db
	 * @param string $type 类型【MONGO，REDIS】
	 * @return dbInit
	 */
	public function init_nosql($db = 'default', $type='MONGO') {
		return $this->dao->nosql->init($type, $db);
	}
	 ...
	 * @param string $type
	 */
	public function init_search($db = 'default', $type='SPHINX') {
		return $this->dao->search->init($type, $db);
	}
}