...
class dbInit extends sqlbuildInit {   
	 ...
	 * @param  array  $where 条件数组 array('username' => 'username')
	 * @param  string $table_name 表名
	 * @param  array  $order 排序
	 * @param  array  $field 显示的字段名
	 * @return bool
	 */
	public function get_one_by_field($where, $table_name, $order=array(), $field='*') {
		if (!is_array($where)) return array();
		$where = $this->build_where($where);
		$order = $this->build_order($order);
		$field = $this->build_field($field);
		$sql = sprintf("SELECT %s FROM %s %s %s LIMIT 1", $field, $table_name, $where, $order);
		$result = $this->query($sql, false);
		if (!$result) return false;
		$r = $this->fetch_assoc($result);
		$this->set_default_link_id(); //设置默认的link_id
	 ...
	 * @param boolean $need_count 是否需要显示数量
	 * @param string $distinct 排除重复的字段
	 * @return array array(数组数据，统计数)
	 */
	public function get_all($table_name, $num = 20, $offest = 0, 
			$where = array(), $order, $field, $need_count=true, $distinct='') {
		$where = $this->build_where($where);
		$limit = $this->build_limit($offest, $num);
		$order = $this->build_order($order);
		$field = $this->build_field($field, $distinct);
		$sql = sprintf("SELECT %s FROM %s %s %s %s", $field, $table_name, $where, $order, $limit);
		$result = $this->query($sql, false);
		if (!$result) return false;
		$temp = array();
	 ...
	 * @param  boolean $distinct 排除重复
	 * @return int
	 */
	public function get_count($table_name, $where = array(), $field='*', $distinct = FALSE) {
		$where = $this->build_where($where);
		$count = 'COUNT('.$field.')';
		if($distinct) {
			$count = 'COUNT(DISTINCT('.$field.'))';
		}
		$sql = sprintf("SELECT %s as count FROM %s %s LIMIT 1", $count, $table_name, $where);
		$result = $this->query($sql, false);
