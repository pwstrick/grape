...
class sqlbuildInit extends dbhandlerInit {
	 ...
	 * @param string $distinct 是否要排除重复
	 * @return string
	 */
	public function build_field($field, $distinct='') {
		if(empty($field)) return '*';
		if(is_array($field)) {
			if(!empty($distinct)) {
				array_unshift($field, 'DISTINCT('.$distinct.')');
			}
